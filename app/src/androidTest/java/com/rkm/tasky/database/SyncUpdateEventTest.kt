...
class SyncUpdateEventTest {
    ...
    fun getSyncUpdateEventWithDetails() = runTest {
        ...
        assertEquals(results.first().uploadPhotos.first().eventId, photos.first().eventId)
        assertEquals(results.first().uploadPhotos.first().filePath, photos.first().filePath)
